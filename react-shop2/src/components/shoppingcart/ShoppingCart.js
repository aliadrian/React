...
const ShoppingCart = ({ products }) => {
  ...
  const shoppingCartTotal = useSelector(
    (state) => state.cart.shoppingCartTotal
  );
    ...
    <div>
      ...
      <div className="p-2 d-flex justify-content-between align-items-center">
        <div className="ml-2">
          ...
          <small className="text-muted">moms</small>
        </div>
        <button onClick={clear} className="btn btn-sm btn-danger">
          Töm varukorgen
        </button>
        <button onClick={handleonClick} className="btn btn-sm white-text unique-color">
          Gå till Kassan
        </button>
      </div>
    </div>
